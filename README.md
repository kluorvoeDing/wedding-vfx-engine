...
- **背景星空閃爍 (Scintillation)**：漂浮背景星辰採用基於空間座標亂數種子的 Shader 算法，模擬真實觀星時星星錯開、隨機發出短暫銳利閃爍的視覺體驗。
- **無縫重建 (Seamless Rebuild)**：切換粒子密度時，舊粒子持續顯示直到新資源就緒後原子交換，過程零黑屏；素材（GLTF 頂點與 LOGO 像素）皆有快取，切換不重新下載。
...
| 按鍵 | 功能 |
|------|------|
| `空白鍵` | 玫瑰 ⇄ LOGO 一鍵切換（4 秒流體轉場，轉場期間按鍵自動忽略） |
| `F` | 進入 / 離開全螢幕 |

兩種狀態（參數已鎖定最佳值）：
1. **白玫瑰待機狀態**：適合於婚禮空檔時間無限期播放，粒子構成的白玫瑰緩慢自轉，背景伴隨閃爍星空。（scale 1.2 / pointSize 2.0 / 密度 5）
